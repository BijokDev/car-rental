...
const Dashboard: React.FC = () => {
    ...
    const handleGalleryPhotoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        setGalleryUploadProgress({ current: 1, total: files.length });
        try {
            for (let i = 0; i < files.length; i++) {
                setGalleryUploadProgress({ current: i + 1, total: files.length });
                const url = await uploadImageToStorage(files[i], 'car-rental-gallery');
            ...
            setGalleryUploadError(error.message || 'Gallery upload failed. Some photos may not have uploaded.');
        } finally {
        ...
        {activeTab === 'gallery' && (
            <div className="space-y-4">
                <div className="flex flex-wrap justify-between items-center gap-3 mb-2">
                    ...
                    <label className={`cursor-pointer ml-auto px-4 py-2.5 rounded-xl shadow-sm flex items-center gap-2 font-medium text-sm transition-all ${uploadingGalleryPhotos ? 'bg-brand-400 text-white cursor-not-allowed pointer-events-none opacity-80' : 'bg-brand-600 text-white hover:bg-brand-700 active:scale-95'}`}>
                        ...
                        <span>
                            {uploadingGalleryPhotos
                                ? `Uploading ${galleryUploadProgress ? `(${galleryUploadProgress.current}/${galleryUploadProgress.total})` : '...'}`
                                : 'Upload Photos'}
                        </span>
                ...
                {/* Error Banner */}
                {galleryUploadError && (
                    <div className="p-3.5 bg-red-50 border border-red-200 rounded-xl flex items-start justify-between gap-2 text-xs text-red-700">
                ...
                {uploadingGalleryPhotos && galleryUploadProgress && (
                    <div className="p-3.5 rounded-xl bg-brand-50 border border-brand-200/70 text-brand-900 shadow-sm">
                        <div className="flex items-center justify-between text-xs font-semibold mb-1.5">
                            <span className="flex items-center gap-2">
                                <Loader2 className="w-4 h-4 animate-spin text-brand-600 shrink-0" />
                                Converting to WebP &amp; uploading photo {galleryUploadProgress.current} of {galleryUploadProgress.total}...
                            </span>
                            <span className="text-brand-600 font-bold">
                                {Math.round(((galleryUploadProgress.current - 1) / galleryUploadProgress.total) * 100)}%
                            </span>
                        </div>
                        <div className="w-full bg-brand-200/60 rounded-full h-1.5 overflow-hidden">
                            <div
                                ...
                                style={{ width: `${Math.max(12, Math.round(((galleryUploadProgress.current - 1) / galleryUploadProgress.total) * 100))}%` }}
                            />
                        </div>
                    </div>
                )}
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
                    {/* In-place Skeleton for uploading photo */}
                    {uploadingGalleryPhotos && (
                        <div className="relative aspect-square rounded-xl border-2 border-dashed border-brand-300 bg-brand-50/60 flex flex-col items-center justify-center text-center p-3 animate-pulse shadow-sm">
                            <div className="w-10 h-10 rounded-full bg-brand-100 flex items-center justify-center mb-2">
                                <Loader2 className="w-5 h-5 animate-spin text-brand-600" />
                            </div>
                            <span className="text-xs font-bold text-brand-900">Uploading photo</span>
                            {galleryUploadProgress && (
                                <span className="text-[11px] text-brand-600 font-semibold mt-0.5">
                                    {galleryUploadProgress.current} of {galleryUploadProgress.total}
                                </span>
                            )}
                            <span className="text-[10px] text-gray-400 mt-1">Converting to WebP</span>
                        </div>
                    )}
